...
pub fn msfx_fn(attr: TokenStream, body: TokenStream) -> TokenStream {
    ...
    if function.sig.inputs.len() == 1 {
        let FnArg::Typed(var) = &function.sig.inputs[0] else {
            panic!("Cannot accept `self` to msfx function")
        };
        ...
        mapping.extend(proc_macro2::TokenStream::from_str(&format!(
            "let {name} = get_unnamed(&arguments, \"{name}\").{ty_fn}()?;"
        )));
    } else if !function.sig.inputs.is_empty() {
        for arg in &function.sig.inputs {
            let FnArg::Typed(var) = arg else {
                panic!("Cannot accept `self` to msfx function")
            };
            let str = var.to_token_stream().to_string();
            let Some((name, ty)) = str.split_once(':') else {
            ...
            if name == "__actual_literal_underscore_lmao" {
                args.extend(proc_macro2::TokenStream::from_str(&format!("{name}, ")));
                mapping.extend(proc_macro2::TokenStream::from_str(&format!(
                    "let {name} = get_named(&arguments, \"_\").{ty_fn}()?;"
                )));
            } else {
                args.extend(proc_macro2::TokenStream::from_str(&format!("{name}, ")));
                mapping.extend(proc_macro2::TokenStream::from_str(&format!(
                    "let {name} = get_named(&arguments, \"{name}\").{ty_fn}()?;"
                )));
            }
        }
    }
