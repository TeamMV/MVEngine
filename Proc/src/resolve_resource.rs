...
pub fn resolve_resource(input: TokenStream) -> TokenStream {
    ...
    let s = lit.value();
    let s = s.strip_prefix('@').expect("resource strings have to start with an @!");
    let (path, name) = s.split_once('/').expect("resource strings have to be in the format '@R.type/name'");
    let (r, t) = path.split_once('.').expect("resource strings have to be in the format '@R.type/name'");
