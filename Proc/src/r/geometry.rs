...
pub fn parse_geometry(entity: &Entity) -> (String, ParsedGeometry) {
    if entity.name().as_str() != "geometry" {
        panic!(
            "Geometry resource must be named geometry, got {}!",
            entity.name()
        );
    }
            ...
            match t.as_str() {
                "shape" => {
                    let Some(XmlValue::Str(val)) = entity.get_attrib("ref") else {
                        panic!("shape geometry needs a shape ref")
                    };
                    return (
                        name.clone(),
                        ParsedGeometry {
                            geom_type: GeomType::Shape,
                ...
                "adaptive" => {
                    let Some(XmlValue::Str(val)) = entity.get_attrib("ref") else {
                        panic!("adaptive geometry needs a adaptive ref")
                    };
                    return (
                        name.clone(),
                        ParsedGeometry {
                            geom_type: GeomType::Adaptive,
                            thingies: vec![val.clone()],
                        },
                    );
                }
