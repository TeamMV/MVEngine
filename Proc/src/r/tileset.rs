...
pub fn parse_tileset(entity: &Entity) -> (String, ParsedTileSet) {
    if entity.name().as_str() != "tileset" {
        panic!(
            "Tileset resource must be named tileset, got {}!",
            entity.name()
        );
    }
    ...
    if let (
        Some(XmlValue::Str(atlas)),
        Some(XmlValue::Str(name)),
        Some(XmlValue::Str(width)),
        Some(XmlValue::Str(height)),
        Some(XmlValue::Str(count)),
    ) = (atlas, name, width, height, count)
    {
        ...
        let mut parsed = ParsedTileSet {
            ...
            width: width
                .parse::<i32>()
                .expect("Tileset width must be a number"),
            height: height
                .parse::<i32>()
                .expect("Tileset height must be a number"),
            count: count
                .parse::<usize>()
                .expect("Tileset count must be a number"),
            linear,
            fps,
            entries: vec![],
        };
            ...
            for entity in entities {
                ...
                if let (Some(XmlValue::Str(name)), Some(XmlValue::Str(index))) = (name, index) {
                    parsed.entries.push((
                        ...
                        index
                            .parse::<usize>()
                            .expect("Tileset entry index must be a number"),
                    ));
                } else {
