...
pub struct IterationTask {
    function: Box<dyn Fn(&mut AnimationState, u32)>,
...
impl IterationTask {
    ...
    where
        F: Fn(&mut AnimationState, u32) + 'static,
    {
        Self {
            current_iter: 0,
            limit,
...
pub struct DelayTask {
    function: Box<dyn Fn(&mut AnimationState, u32, u32)>,
    state: AnimationState,
    init_time: u128,
    duration: u32,
    delay: u32,
...
impl DelayTask {
    ...
    where
        F: Fn(&mut AnimationState, u32, u32) + 'static,
    {
        Self {
            function: Box::new(function),
            state,
...
impl TimingTask for DelayTask {
    ...
    fn iteration(&mut self, dt: f32, frame: u64) {
        ...
        if ms - self.init_time > (self.delay * (self.current_iter + 1)) as u128 {
            (self.function)(&mut self.state, (ms - self.init_time) as u32, self.current_iter);
            self.current_iter += 1;
        }
    }
}
...
impl AnimationState {
    ...
    pub(crate) fn element(em: ElementAnimationInfo) -> Self {
        Self {
            background: None,
            element: Some(em),
        }
