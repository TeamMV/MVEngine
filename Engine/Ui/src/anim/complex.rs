...
pub trait UiElementAnimationStub {
    fn play(&self, elem: &mut UiElement, duration: u32, fill_mode: FillMode, animation_mode: AnimationMode);
}
...
impl UiElementAnimationStub for SimpleAnimation {
    fn play(&self, elem: &mut UiElement, duration: u32, fill_mode: FillMode, animation_mode: AnimationMode) {
        anim::animate_self(elem, &self.target_style, duration, self.easing.clone(), fill_mode, animation_mode);
    }
}
...
impl KeyframeAnimation {
    ...
    fn animate_next_keyframe<F>(&self, index: usize, complete_duration: u32, elem: &mut UiElement, animation_mode: AnimationMode, on_finish: Arc<DangerousCell<F>>) where F: FnMut() + 'static {
        unsafe {
            ...
            anim::animate_self(elem, &keyframe.style, duration, keyframe.easing.clone(), FillMode::Keep, animation_mode.clone());
            ...
            TIMING_MANAGER.request(DelayTask::new(duration, |_, _| {}, AnimationState::empty()), Some(Box::new(move || {
...
impl UiElementAnimationStub for KeyframeAnimation {
    fn play(&self, elem: &mut UiElement, duration: u32, fill_mode: FillMode, animation_mode: AnimationMode) {
        ...
        self.animate_next_keyframe(0, duration, elem, animation_mode, Arc::new(DangerousCell::new(move || {
            match fill_mode {
                FillMode::Keep => {}
                FillMode::Revert => {
...
impl KeyframeAnimationBuilder {
    ...
    pub fn next_keyframe(mut self, modifier: fn(&mut UiStyle), easing: Option<Easing>, duration: Option<f32>) -> Self {
        ...
        self.keyframes.push(Keyframe::new(self.last_style.clone(), easing.unwrap_or(anim::easing(EasingGen::linear(), EasingMode::In)), duration));
