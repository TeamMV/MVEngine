...
void main() {
    ...
    outColor = texture(sampler2D(tex, sam), vec2(xpos, uv.y));

    // Mix in some random interference for lines
    outColor.rgb = mix(outColor.rgb, vec3(rand(vec2(uv.y * time))), noise * 0.3).rgb;

    // Apply a line pattern every 4 pixels
    if (floor(mod(fTexCoord.y * 0.25, 2.0)) == 0.0)
    {
        outColor.rgb *= 1.0 - (0.15 * noise);
    }

    // Shift green/blue channels (using the red channel)
    outColor.g = mix(outColor.r, texture(sampler2D(tex, sam), vec2(xpos + noise * 0.05, uv.y)).g, 0.25);
    outColor.b = mix(outColor.r, texture(sampler2D(tex, sam), vec2(xpos - noise * 0.05, uv.y)).b, 0.25);
}