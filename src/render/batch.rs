...
use crate::render::shared::{RenderProcessor2D, Shader, Texture, TextureRegion};

pub(crate) const FLOAT_BYTES: u8 = 4;
...
pub mod batch_layout_2d {
    ...
    pub(crate) const ROTATION_ORIGIN_SIZE: u8 = 2;
...
pub(crate) trait BatchGen : Default {
    fn get_render_mode(&self) -> u8;
    fn gen_indices(&self, amt: u16, offset: u32, indices: &Vec<u32>);
...
impl Default for RegularBatch {
    fn default() -> Self { RegularBatch }
}
...
impl BatchGen for RegularBatch {
    ...
    fn gen_indices(&self, amt: u16, offset: u32, indices: &Vec<u32>) {
        if amt == 4 {
            indices[offset * 6 + 0] = 0 + offset * 4;
            indices[offset * 6 + 1] = 1 + offset * 4;
            indices[offset * 6 + 2] = 2 + offset * 4;
            indices[offset * 6 + 3] = 0 + offset * 4;
            indices[offset * 6 + 4] = 2 + offset * 4;
            indices[offset * 6 + 5] = 3 + offset * 4;
        } else {
            indices[offset * 6 + 0] = 0 + offset * 4;
            indices[offset * 6 + 1] = 1 + offset * 4;
            indices[offset * 6 + 2] = 2 + offset * 4;
...
impl Default for StrippedBatch {
    fn default() -> Self { StrippedBatch }
}
...
struct Batch2D {
    generator: dyn BatchGen,
    data: Vec<f32>,
    indices: Vec<u32>,
    textures: [Option<Rc<Texture>>; 17],
    tex_ids: [u32; 17],
    shader: Rc<Shader>,
    vbo: u32,
    ibo: u32,
...
impl Batch2D {
    pub(crate) fn new<T: BatchGen>(size: u32, processor: &impl RenderProcessor2D<_>, shader: Rc<Shader>) -> Self {
        Batch2D {
            generator: T::default(),
            data: Vec::with_capacity(size as usize * batch_layout_2d::VERTEX_SIZE_FLOATS),
            indices: Vec::with_capacity(size as usize * 6),
            textures: [None; 17],
            tex_ids: [0; 17],
            shader,
            vbo: processor.gen_buffer_id(),
            ibo: processor.gen_buffer_id(),
            size,
            vert_count: 0,
            obj_count: 0,
            next_tex: 0,
    ...
    fn add_vertex(&mut self, vertex: &Vertex2D) {
        for i in 0..batch_layout_2d::VERTEX_SIZE_FLOATS {
            self.data[i + (self.vert_count * batch_layout_2d::VERTEX_SIZE_FLOATS)] = vertex.get(i);
        }
        self.vert_count.inc();
    }
    ...
    pub(crate) fn add_vertices(&mut self, vertices: &VertexGroup<Vertex2D>) {
        ...
        self.generator.gen_indices(vertices.len as u16, self.obj_count, &self.indices);
    ...
    pub(crate) fn add_texture(&mut self, texture: Rc<Texture>) -> u32 {
        if self.full_tex {
            return 0;
        }

        for i in 0..17 {
            if let Some(tex) = self.textures[i].borrow() {
        ...
        self.textures[self.next_tex] = Some(texture);
        self.tex_ids[self.next_tex] = self.next_tex + 1;
        self.next_tex += 1;
    ...
    pub(crate) fn render(&mut self, processor: &impl RenderProcessor2D<_>) {
        processor.process_data(&mut self.textures, &self.tex_ids, &self.indices, &self.data, self.vbo, self.ibo, &mut self.shader, processor.adapt_render_mode(self.generator.get_render_mode()));
        self.force_clear();
    }

    pub(crate) fn set_shader(&mut self, shader: Rc<Shader>) {
        self.shader = shader;
    }
}
...
impl VertexGroup<Vertex2D> {
    ...
    pub(crate) fn get(&self, index: u8) -> &Vertex2D {
        ...
        self.vertices[index as usize].borrow()
    }

    pub(crate) fn get_mut(&mut self, index: u8) -> &mut Vertex2D {
        assert!(index >= 3 && index <= 4);
        self.vertices[index as usize].borrow_mut()
    }
...
impl BatchController2D {
    pub(crate) fn new(mut shader: Rc<Shader>, batch_limit: u32) -> Self {
