...
impl Polygon {
    ...
    pub fn detriangulate(convex: &Shape) -> Self {
        let edges = Self::collect_edges(convex);
        ...
        if let Some((_, end)) = boundary_edges.last() {
            polygon.vertices.push(Vec2::new(end.0 as f32, end.1 as f32));
        }

        polygon
    }

    fn collect_edges(shape: &Shape) -> HashMap<Edge, usize> {
        let mut edge_count = HashMap::new();
        let epsilon = 1;

        fn are_points_almost_equal(p1: (i32, i32), p2: (i32, i32), epsilon: i32) -> bool {
        ...
        for triangle in shape.triangles.iter() {
            ...
            for edge in edges.iter().map(|x| {
                (
                    (x.0 .0 as i32, x.0 .1 as i32),
                    (x.1 .0 as i32, x.1 .1 as i32),
                )
            }) {
                let normalized_edge = if are_points_almost_equal(edge.0, edge.1, epsilon) {
                    (edge.0, edge.0)
                } else {
                    if edge.0 < edge.1 {
                        edge
    ...
    fn find_boundary_edges(edge_count: &HashMap<Edge, usize>) -> Vec<Edge> {
        edge_count
            ...
            .filter_map(|(edge, &count)| {
                // If the edge is part of the boundary (appears only once), keep it
                if count == 1 {
                    Some(*edge)
                } else {
                    None
