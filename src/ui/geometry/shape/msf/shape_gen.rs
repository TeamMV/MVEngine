...
fn parse_transform(parsed_struct: &ParsedStruct) -> Result<Transform, String> {
    ...
    for (key, value) in &parsed_struct.values {
        ...
        match skey {
            "r" => {
                if let StructValue::Number(num) = value {
                    transform.rotation = num.as_f32().to_radians();
                } else {
...
fn gen_tri(parsed_struct: ParsedStruct) -> Result<Shape, String> {
    let mut p1 = (0, 0);
    let mut p2 = (0, 0);
    let mut p3 = (0, 0);
    let mut transform = Transform::new();
...
fn gen_rect(parsed_struct: ParsedStruct) -> Result<Shape, String> {
    let mut p1 = (0, 0);
    let mut p2 = (0, 0);
    let mut wh = (0, 0);
    let mut is_wh = false;
    ...
    pub fn generate(ast: Ast) -> Result<Shape, String> {
        ...
        for command in ast {
            match command {
                ...
                Command::Call(function, params) => match function.as_ref() {
                    "transform" => {
                        let current = vars
                    ...
                    "combine" => {
                        let first = params
                            .get(0)
                            .ok_or("combine needs one other shape".to_string())?;
                    ...
                    "modifier" => {
                        let modifier = params
                            .get(0)
                            .ok_or("modifier needs one modifier name argument".to_string())?;
