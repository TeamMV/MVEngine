use crate::graphics::tileset::{ClockingFramePump, FramePumpRange, LoopingRangeFramePump, Pump, TileSet};
...
impl<'a> GlobalAnimation<'a> {
    pub fn new(tile_set: &'a TileSet, tile_set_res: usize, range: impl RangeBounds<usize>, fps: u16) -> Self {
        ...
        let last = pump.pump();
        Self { pump, last, tile_set: tile_set_res, fps, range: fprange }
    }
...
impl ResourceSavable for GlobalAnimation<'_> {
    ...
    fn load_res(loader: &mut impl Loader, resources: &impl UiResources) -> Result<Self, String> {
        ...
        if let Some(tileset) = resources.resolve_tileset(tile_set) {
            ...
            Ok(GlobalAnimation::<'static>::new(static_set, tile_set, range, fps))
        } else {
