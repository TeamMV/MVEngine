...
use crate::rendering::backend::image::{AccessFlags, Image, ImageAspect, ImageFormat, ImageLayout, ImageTiling, ImageType, ImageUsage, MVImageCreateInfo};
...
impl XRendererCore {
    ...
    pub fn new(window: &Window, create_info: RendererCreateInfo) -> Option<Self> {
        let device = Device::new(Backend::Vulkan, MVDeviceCreateInfo {
            ...
            green_eco_mode: create_info.flags.contains(RendererCreateInfoFlags::GREEN_ECO_MODE),
            device_extensions: Extensions::DESCRIPTOR_INDEXING,
        }, window.get_handle())?;

        let vsync = create_info.flags.contains(RendererCreateInfoFlags::VSYNC);

        let swapchain = Self::create_swapchain(device.clone(), None, window.info.width, window.info.height, vsync, create_info.frames_in_flight);

        let mut command_buffers = Vec::new();

        for i in 0..swapchain.get_image_count() {
            command_buffers.push(CommandBuffer::new(device.clone(), MVCommandBufferCreateInfo {
    ...
    pub fn load_shader(&self, name: &str, flavor: ShaderFlavor, source: &str) -> Result<Shader, shaderc::Error> {
    ...
    fn create_swapchain(device: Device, previous: Option<Swapchain>, width: u32, height: u32, vsync: bool, max_frames_in_flight: u32) -> Swapchain {
    ...
    pub fn recreate(&mut self, width: u32, height: u32, vsync: bool, frames_in_flight: u32) {
        ...
        self.swapchain = Self::create_swapchain(self.device.clone(), Some(self.swapchain), width, height, vsync, frames_in_flight);

        self.command_buffers.clear();
        for i in 0..self.swapchain.get_image_count() {
            self.command_buffers.push(CommandBuffer::new(self.device.clone(), MVCommandBufferCreateInfo {
                level: CommandBufferLevel::Primary,
    ...
    pub fn end_draw(&mut self) -> Result<(), SwapchainError> {
        ...
        self.swapchain.submit_command_buffer(cmd, self.swapchain.get_current_image_index())
    }
